...
import 'package:app_admin/components/section_data_source.dart';
...
class Sections extends ConsumerStatefulWidget {
  const Sections({Key? key}) : super(key: key);

  @override
  ConsumerState<Sections> createState() => _SectionsState();
}

class _SectionsState extends ConsumerState<Sections> {
  ...
  final List<DataColumn> _columns = [
    ...
    const DataColumn(label: Text('Section Name')),
//     const DataColumn(label: Text('Questions Amount')),
      ...
      body: CardWrapper(
        child: FirestoreQueryBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot, _) {
            ...
            sectionList =
                snapshot.docs.map((e) => Section.fromFirestore(e)).toList();
            DataTableSource source =
                SectionDataSource(context, sectionList, ref);
            return Padding(
              padding: const EdgeInsets.only(bottom: 10),
              child: PaginatedDataTable2(
                rowsPerPage: _itemPerPage - 1,
                source: source,
                header: Text('All Sections',
    ...
    return Padding(
      ...
      child: TextButton.icon(
        ...
        onPressed: () => CustomDialogs.openResponsiveDialog(
          context,
          widget: const SectionForm(sections: null),
  ...
  _openCategoryDialog() async {
    await FirebaseService().getCategories().then(
      (List<Category> cList) {
        showDialog(
          context: context,
          builder: (context) {
                      ...
                      onTap: () {
                        setState(() {
                          ...
                          _query = firestore
                              ...
                              .where('parent_id', isEqualTo: cList[index].id);
                        });
