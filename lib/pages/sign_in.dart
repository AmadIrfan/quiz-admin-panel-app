...
class _SignInPageState extends ConsumerState<SignInPage> {
  ...
  void _handleSignIn() async {
    if (formKey.currentState!.validate()) {
      ...
      _btnCtlr.start();
      await AuthService()
          .loginWithEmailPassword(emailCtrl.text, passwordCtrl.text)
        ...
        if (user != null) {
          ...
          if (role == UserRoles.admin || role == UserRoles.editor) {
            ...
            if (!mounted) return;
            openCustomDialog(
                context, 'The email is not authorized as an admin', '');
          }
        } else {
          _btnCtlr.reset();
          debugPrint('SignInErorr');
          openCustomDialog(context, 'Sign In Error! Please try again.',
  ...
  //   await AuthService().loginAnnonumously().then((value) => NextScreen().nextScreenCloseOthersAnimation(context, const HomePage()));
  // }
